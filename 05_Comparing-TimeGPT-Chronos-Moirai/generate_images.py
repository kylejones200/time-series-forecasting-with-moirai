...
df_long = ok_data.melt(
    id_vars=["State", "MSN"], value_vars=year_cols, var_name="Year", value_name="Value"
)
...
def forecast_timegpt(ts_data, horizon=24, api_key=None):
    ...
    return {
        ...
        "upper_80": forecast["TimeGPT-hi-80"].values
...
results["Chronos"] = {**chronos_metrics, "Time": chronos_results["time"]}

# Moirai
moirai_metrics = calculate_metrics(ts_test.values, moirai_results["forecast"])
results["Moirai"] = {**moirai_metrics, "Time": moirai_results["time"]}
...
ax1.fill_between(
    ts_test.index,
    chronos_results["lower"],
    chronos_results["upper"],
    alpha=0.2,
    color="#1f77b4",
)
...
ax1.fill_between(
    ts_test.index,
    moirai_results["lower"],
    moirai_results["upper"],
    alpha=0.2,
    color="#ff7f0e",
)
