...
def get_the_groups_you_compared() -> None:
    ...
    for g1, g2 in itertools.combinations(groups, 2):
        ...
        logrank_results.append({'Group 1': g1, 'Group 2': g2, 'Chi2': result.test_statistic, 'p-value': result.p_value, '-log2(p)': -np.log2(result.p_value) if result.p_value > 0 else float('inf')})
...
def load_your_dataset() -> None:
    ...
    spell_df = df.dropna(subset=['usage_group']).groupby(['ccode', 'techname', 'usage_group']).agg(start_year=('year', 'min'), end_year=('year', 'max')).reset_index()
...
def step_1_mark_technologies_as_used_based_on_correc() -> None:
    ...
    spell_df = arms_df.dropna(subset=['usage_group']).groupby(['ccode', 'techname', 'usage_group']).agg(start_year=('year', 'min'), end_year=('year', 'max')).reset_index()
...
def load_long_format_data() -> None:
    ...
    tech_groups = {t: df for t, df in spells[spells['techtype'].isin(top_types)].groupby('techtype')}
