...
def detect_start_of_each_new_spell_of_use() -> None:
    import pandas as pd
...
def remove_top_and_right_spines() -> None:
    kmf = KaplanMeierFitter()
...
def load_dataset() -> None:
    ...
    spells = df.groupby(['ccode', 'techname', 'spell_id']).agg(start_year=('year', 'min'), end_year=('year', 'max')).reset_index()
...
def run_pairwise_log_rank_tests() -> None:
    ...
    for i in range(len(labels)):
        for j in range(i + 1, len(labels)):
            ...
            result = logrank_test(g1['duration_years'], g2['duration_years'], event_observed_A=g1['event_observed'], event_observed_B=g2['event_observed'])
            logrank_results.append({'Period 1': labels[i], 'Period 2': labels[j], 'Chi2': result.test_statistic, 'p-value': result.p_value, '-log2(p)': -np.log2(result.p_value) if result.p_value > 0 else float('inf')})
...
def load_and_preprocess_the_dataset() -> None:
    df = pd.read_csv('cow_arms_tech_long.csv')
    ...
    spells = df.groupby(['ccode', 'techname', 'techtype', 'spell_id']).agg(start_year=('year', 'min'), end_year=('year', 'max')).reset_index()
    ...
    small_arms['region'] = small_arms['ccode'].apply(lambda x: 'NATO' if x in nato_ccodes else 'Africa' if x in africa_ccodes else 'Other')
    ...
    cph.fit(subset[['duration_years', 'event_observed', 'is_nato']], duration_col='duration_years', event_col='event_observed')
    ...
    logrank = logrank_test(nato['duration_years'], africa['duration_years'], event_observed_A=nato['event_observed'], event_observed_B=africa['event_observed'])
...
def extend_region_mapping_to_include_europe() -> None:
    europe_ccodes = [200, 205, 220, 230, 255, 265, 300, 305, 310, 317, 325, 331, 335, 350, 359, 364, 367, 371, 372, 375, 380, 385, 390]

    small_arms['region'] = small_arms['ccode'].apply(lambda x: 'NATO' if x in nato_ccodes else 'Africa' if x in africa_ccodes else 'Europe' if x in europe_ccodes else 'Other')
...
def main() -> None:
    detect_start_of_each_new_spell_of_use()
    remove_top_and_right_spines()
    load_dataset()
    run_pairwise_log_rank_tests()
    load_and_preprocess_the_dataset()
    notebook_step_007()
    load_and_preprocess()
    extend_region_mapping_to_include_europe()
